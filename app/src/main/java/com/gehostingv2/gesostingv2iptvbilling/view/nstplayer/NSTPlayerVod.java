...
//        handler.removeMessages(MESSAGE_FADE_OUT);
    ...
    public void play(String url, int opened_stream_id, String videoExtension) {
        ...
        if (playerSupport) {
            $.id(R.id.app_video_loading).visible();
            videoView.setVideoPath(url+opened_stream_id+"."+videoExtension);
            videoView.start();
        }
    }
...
//            searchEditText.setVisibility(View.GONE);
    ...
    /**
     * using constants in NSTPlayerVod,eg: NSTPlayerVod.SCALETYPE_FITPARENT
     * @param scaleType
     */
...
//                LinearLayout seekbarLinearLayout  = (LinearLayout) activity.findViewById(R.id.ll_seekbar_time);
                ...
                if(channelListView.getVisibility()==View.VISIBLE){
                    ...
                    bottomBoxRelativeLayout.setVisibility(View.GONE);
